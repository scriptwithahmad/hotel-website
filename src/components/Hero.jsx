import Link from "next/link";
import React from "react";

const Hero = () => {
  return (
    <>
      <div className="bg-[url('/image/sign/hero1.webp')] h-[65vh] bg-center bg-cover relative">
        <div className=" flex items-center justify-center flex-col bg-[#6F5243] absolute -bottom-6 right-28 p-2 rounded-full h-72 w-72">
          <h1 className="  text-white text-[35px] font-bold mb-4">
            Book Now!!
          </h1>
          <Link
            className=" globalShadow bg-white text-gray-600 px-5 py-2 font-semibold"
            href={"https://tableagent.com/dundee/yamm-buffet/table-search/"}
          >
            RESERVED NOW
          </Link>
        </div>
      </div>

      <div className="bg-[url('/image/sign/hero2.webp')] h-[100vh] bg-center bg-cover">
        <div className=" flex items-center justify-center flex-col pt-12">
          <h2 className=" text-white text-[36px] font-bold mb-8">REVIEWS</h2>
          <div className=" flex items-center gap-5">
            <div className=" bg-white h-7 w-7 rounded-full">
              <img
                alt="google image"
                className=" h-full w-full rounded-full p-1.5"
                src="data:image/png;base64,"
              />
            </div>
            <span className=" text-white font-semibold text-[35px]">4.1</span>
            <div>
              <h2 className=" text-white text-2xl">Yamm Buffet</h2>
              <span className=" text-white text-xl">709</span>
            </div>
          </div>
        </div>

        <div className=" max-w-[1000px] m-auto grid grid-cols-3 gap-6">
          <div className=" bg-white flex flex-col justify-center items-center gap-3 px-12 py-8">
            <img
              alt="image here"
              className=" h-20 w-20 mb-4"
              src="https://lh3.googleusercontent.com/a/ACg8ocLv3L3Y15Zgl6-J9MDWz_ZRDHEHXA0cFtLZ9YJ-jDAC=s120-c-rp-mo-br100"
            />
            <div className=" text-yellow-400 mb-3">
              <i class="fa-solid fa-star"></i>
              <i class="fa-solid fa-star"></i>
              <i class="fa-solid fa-star"></i>
              <i class="fa-solid fa-star"></i>
              <i class="fa-solid fa-star"></i>
            </div>
            <p className="text-center">
              "Excellent food great range of different dishes 😋"
            </p>
            <div className=" flex items-center gap-1">
              <img
                alt="google image"
                className=" h-6 w-6 rounded-full p-1.5"
                src="data:image/png;base64,"
              />
              <div>
                <span className=" text-xs">John Doe {" - "}</span>
                <span className=" text-xs"> 10/01/2024 </span>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className=" max-w-[1200px] m-auto">
        <div className=" text-center my-8">
          <h2 className=" text-2xl mb-6 font-semibold">SOCIAL</h2>
          <i class="fa-brands fa-facebook text-2xl"></i>
        </div>

        <div className=" flex gap-8 items-center justify-center my-4"> 
            <img className=" w-1/2" src="/image/sign/hero3.webp" alt="" />
            <div>
                <h1 className=" text-3xl font-semibold mb-5 ">Heading</h1>
                <p className=" text-gray-500 text-lg leading-[1.6]">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Corporis laboriosam, ipsam ex ab perspiciatis aliquam, reiciendis maiores aliquid asperiores ducimus est labore molestiae pariatur minus esse animi maxime at aut?</p>
            </div>
        </div>

      </div>
    </>
  );
};

export default Hero;
